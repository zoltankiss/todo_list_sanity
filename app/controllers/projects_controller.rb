class ProjectsController < ApplicationController
  ...
  def share
    email = params['project']['user_email_to_share_with']
    user_to_share_with = User.where(email: email).first

    if user_to_share_with.nil?
      redirect_to(
        projects_path,
        notice: "Sorry we could not find that user's email in our database. <a href='/help'>more help</a>"
      )
      return
    end
